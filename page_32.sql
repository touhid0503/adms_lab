CREATE TABLE Worker1 (
    WORKER_ID INT PRIMARY KEY,
    FIRST_NAME VARCHAR(50),
    LAST_NAME VARCHAR(50),
    SALARY INT,
    DEPARTMENT VARCHAR(50),
    JOINING_DATE DATETIME
);

INSERT INTO Worker1 VALUES
(1,'Rana','Hamid',100000,'HR','2024-02-20'),
(2,'Sanjoy','Saha',80000,'Admin','2024-06-11'),
(3,'Mahmudul','Hasan',300000,'HR','2024-02-20'),
(4,'Asad','Zaman',500000,'Admin','2024-02-20'),
(5,'Sajib','Mia',500000,'Admin','2024-06-11'),
(6,'Alamgir','Kabir',200000,'Account','2024-06-11'),
(7,'Foridul','Islam',75000,'Account','2024-01-20'),
(8,'Keshob','Ray',90000,'Admin','2024-04-11');

select * from Worker1

SELECT LEFT(FIRST_NAME,3) FROM Worker1;

SELECT * FROM Worker1
WHERE JOINING_DATE BETWEEN '2024-02-01' AND '2024-03-31';

SELECT * FROM Worker1
WHERE DATEDIFF(MONTH, JOINING_DATE, GETDATE()) >= 6;

UPDATE Worker1
SET SALARY = CASE 
    WHEN JOINING_DATE < '2024-04-11' AND DEPARTMENT='Admin'
        THEN SALARY * 1.10
    ELSE SALARY * 1.05
END;

SELECT * FROM Worker1
WHERE FIRST_NAME IN ('Rana','Sajib');

SELECT * FROM Worker1
WHERE FIRST_NAME NOT IN ('Rana','Sajib');

SELECT * FROM Worker1
WHERE FIRST_NAME LIKE '%a%';

SELECT * FROM Worker1
WHERE FIRST_NAME LIKE 'k%';

SELECT * FROM Worker1
WHERE FIRST_NAME LIKE '______r';

SELECT CHARINDEX('n', FIRST_NAME)
FROM Worker1
WHERE FIRST_NAME = 'Sanjoy';

SELECT DEPARTMENT, AVG(SALARY)
FROM Worker1
GROUP BY DEPARTMENT;

SELECT *
FROM Worker1
WHERE SALARY IN (
    SELECT MAX(SALARY) FROM Worker1 GROUP BY DEPARTMENT
    UNION
    SELECT MIN(SALARY) FROM Worker1 GROUP BY DEPARTMENT
);

SELECT CHARINDEX('r', FIRST_NAME)
FROM Worker1
WHERE FIRST_NAME = 'Rana';

SELECT RTRIM(FIRST_NAME)
FROM Worker1;

SELECT DISTINCT FIRST_NAME, LEN(FIRST_NAME)
FROM Worker1;

SELECT REPLACE(FIRST_NAME, 'a', 'A') 
FROM Worker1;

